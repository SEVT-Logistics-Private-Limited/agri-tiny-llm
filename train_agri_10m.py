"""
AgriTinyGPT Rung 3 - ~9.92M parameters
6 domains, with hydroponics expanded to comprehensive A-Z coverage (all system
types, full nutrient/deficiency list, 5 crop profiles, light/CO2, water treatment,
pests). Uses lower LR + cosine decay + gradient clipping + best-checkpoint saving
for training stability at this size (a fixed high LR caused instability, fixed here).
"""
...
with open("agri_corpus_10m.txt", "r", encoding="utf-8") as f:
    text = f.read()
...
n_head     = 8
n_layer    = 4
lr, steps, batch_size = 1e-3, 4000, 32
...
torch.save({"model": model.state_dict(), "stoi": stoi, "itos": itos,
           ...
           "agri_tiny_llm_10m.pt")
print("saved agri_tiny_llm_10m.pt")
